# Modified from https://github.com/terarachang/MemPi
# coding=utf-8
...
from utils.utils import get_attributes, set_attributes
...
class L0Mask(nn.Module):
    ...
    def init_weights(self):
        ...
        # init.constant_(self.mask_scores, val=100.)
...
class MaskedLinear(nn.Linear):
    ...
    def forward(self, input: torch.tensor):
        ...
        masked_weight = self.produce_mask().to(self.weight.device) * self.weight.reshape(
...
def set_reverse_mask_mode(model, reverse_mask):
    for l in range(model.config.n_layer):
        attr_str = f"{model.attr_dict['transformer_layer']}.{l}.{model.attr_dict['ffn_out']}.mask"
        mask_module = get_attributes(model, attr_str)
        mask_module.reverse_mask = reverse_mask